	...
	OUT std::vector<cMtlTex*>& vecHiddenMtlTex, OUT std::vector<LPD3DXMESH>& vecHiddenMesh)
{
	...
	while (!feof(fp))
	{
		...
		else if (szBuf[0] == 'm')
		{
			char szMtlPath[1024];
			sscanf_s(szBuf, "%*s %s", szMtlPath, 1024);
			//LoadMtlLib(szMtlPath, vecMtlTex, vecHiddenMtlTex, vecSkyBoxMtlTex);
		...
		else if (szBuf[0] == 'f')
		{
			...
			for (int i = 0; i < 3; ++i)
			{
				...
				else if (StartsWith(StringToChar(sMtlName), "SkyBox"))
					vecHiddenVertex.push_back(v);
				else
					vecVertex.push_back(v);
			}
	...
	OUT std::vector<cMtlTex*>& vecSkyBoxMtlTex, OUT LPD3DXMESH& SkyBoxmesh)
{
	...
	std::vector<ST_PNT_VERTEX>	vecSkyBoxVertex;
	std::vector<DWORD>			vecSkyBoxAttr;
	...
	while (!feof(fp))
	{
		...
		else if (szBuf[0] == 'f')
		{
			...
			{
				vecSkyBoxAttr.push_back(m_mapSkyBoxMtlTex[sMtlName]->GetAttrID());
			}
			else
			{
				vecAttr.push_back(m_mapMtlTex[sMtlName]->GetAttrID());
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////////////
	D3DXCreateMeshFVF(vecSkyBoxVertex.size() / 3,
		vecSkyBoxVertex.size(),
		D3DXMESH_MANAGED | D3DXMESH_32BIT,
		ST_PNT_VERTEX::FVF,
		g_pD3DDevice,
		&SkyBoxmesh);

	ST_PNT_VERTEX* pV3 = NULL;
	SkyBoxmesh->LockVertexBuffer(0, (LPVOID*)&pV3);
	memcpy(pV3, &vecSkyBoxVertex[0], sizeof(ST_PNT_VERTEX) * vecSkyBoxVertex.size());
	SkyBoxmesh->UnlockVertexBuffer();

	int* pI3 = NULL;
	SkyBoxmesh->LockIndexBuffer(0, (LPVOID*)&pI3);
	for (int i = 0; i < vecSkyBoxVertex.size(); ++i)
	{
		pI3[i] = i;
	}
	...
	DWORD* pA3 = NULL;
	SkyBoxmesh->LockAttributeBuffer(0, &pA3);
	memcpy(pA3, &vecSkyBoxAttr[0], sizeof(DWORD) * vecSkyBoxAttr.size());
	SkyBoxmesh->UnlockAttributeBuffer();

	std::vector<DWORD> vecAdj3(vecSkyBoxVertex.size());
	SkyBoxmesh->GenerateAdjacency(0.0f, &vecAdj3[0]);

	SkyBoxmesh->OptimizeInplace(
		D3DXMESHOPT_COMPACT |
		D3DXMESHOPT_ATTRSORT |
		D3DXMESHOPT_VERTEXCACHE,
		&vecAdj3[0],
...
void cObjLoader::LoadMtlLib(IN char * szFilename, OUT std::vector<cMtlTex*>& vecMtlTex, OUT std::vector<cMtlTex*>& vecHiddenMtlTex, OUT std::vector<cMtlTex*>& vecSkyBoxMtlTex)
{
	std::string sMtlName;

	FILE* fp = NULL;
	OBJECT_TYPE objType = OBJ_NONE;
	fopen_s(&fp, szFilename, "r");
	...
	while (!feof(fp))
	{
		...
		else if (StartsWith(szBuf, "newmtl"))
		{
			...
			{
				m_mapSkyBoxMtlTex[sMtlName] = new cMtlTex;
				m_mapSkyBoxMtlTex[sMtlName]->SetAttrID(vecSkyBoxMtlTex.size());
				vecSkyBoxMtlTex.push_back(m_mapSkyBoxMtlTex[sMtlName]);
				objType = OBJ_SKYBOX;
			}
			else
			{
		...
		else if (szBuf[0] == 'K')
		{
			if (szBuf[1] == 'a')
			{
				float r, g, b;
				sscanf_s(szBuf, "%*s %f %f %f", &r, &g, &b);
				if (objType == OBJ_FLOWER)
				{
					D3DMATERIAL9& stMtl = m_mapHiddenMtlTex[sMtlName]->GetMtl();
					stMtl.Ambient = D3DXCOLOR(r, g, b, 1.0f);
				}
				else if (objType == OBJ_SKYBOX)
				{
					D3DMATERIAL9& stMtl = m_mapSkyBoxMtlTex[sMtlName]->GetMtl();
					stMtl.Ambient = D3DXCOLOR(r, g, b, 1.0f);
				}
				else if (objType == OBJ_NONE)
				{
					D3DMATERIAL9& stMtl = m_mapMtlTex[sMtlName]->GetMtl();
					stMtl.Ambient = D3DXCOLOR(r, g, b, 1.0f);
				}
		...
		else if (szBuf[0] == 'm')
		{
			...
			{
				m_mapSkyBoxMtlTex[sMtlName]->SetTexture(g_pTextureManager->GetTexture(szTexturePath));
			}
			else if (objType == OBJ_NONE)
			{
				m_mapMtlTex[sMtlName]->SetTexture(g_pTextureManager->GetTexture(szTexturePath));
