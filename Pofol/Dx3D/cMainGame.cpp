...
void cMainGame::Setup()
{
	...
	m_pFiona->Setup();
	fionaAniNum = 15;
	m_pFiona->GetSkinnedMesh()->SetAnimationIndex(&fionaAniNum);
...
void cMainGame::Update()
{
	...
	if (m_pFiona)
	{
		m_pFiona->Update(m_pCharController->GetAniIndex(), m_pCharController->GetWorldTM());
	}
	...
	case WM_KEYDOWN:
		{
			if (wParam == VK_SPACE)
			{
				m_pFiona->GetSkinnedMesh()->SetAnimationIndex(&aniNum);
				aniNum++;
			}
			if (wParam == 'L')
			{
