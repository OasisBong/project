...
void cSkinnedMesh::UpdateAndRender(D3DXMATRIXA16* pmat)
{
	...
	{
		m_pAnimController->AdvanceTime(g_pTimeManager->GetDeltaTime() * m_fAniRenderTime, NULL);
	}
...
void cSkinnedMesh::SetAnimationIndex( int* nIndex )
{
	...
	if (*nIndex > m_pAnimController->GetMaxNumAnimationSets())
	{
		*nIndex = 0;
	}
...
void cSkinnedMesh::FindFrame(LPSTR FrameName, ST_BONE* pBone)
{
	LPSTR n = NULL;
	n = pBone->Name;
	if (pBone || (n == NULL))
	{
		//m_vecname.push_back(n);
		if ((n != NULL) && !strcmp(n, FrameName))
		{
		...
		else
		{
			if (pBone->pFrameFirstChild)
			{
				FindFrame(FrameName, (ST_BONE*)pBone->pFrameFirstChild);
			}

			if (pBone->pFrameSibling)
			{
				FindFrame(FrameName, (ST_BONE*)pBone->pFrameSibling);
			}
		}
	}
}